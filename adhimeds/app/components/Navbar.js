...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const storeEmail =
      ...
      sessionStorage.getItem("storeEmail");
  ...
  const fetchStoreDetails = async (email) => {
    try {
      const response = await fetch(
        `/api/store/by-email?email=${email}`
      );
    ...
    async function fetchAllProducts() {
      ...
      try {
        ...
        const shopsRes = await fetch(
          `${SERVERURL}/api/app/medisoft/shops`,
          {
        ...
        const targetShop = shops.find(
          (shop) =>
            ...
            targetStoreName.toUpperCase()
        );

        if (!targetShop) {
          setError(
            `Store "${targetStoreName}" not found`
          );
          setLoading(false);
          return;
        }
        ...
        console.log(
          `✅ Found target shop: ${targetShop.name} (${targetShop.shopid})`
        );
        ...
        const targetOption = storeOptions.find(
          (opt) => !opt.isDisabled
        );
        ...
        console.log(
          `📦 Fetching products for shop: ${targetShop.shopid}`
        );

      const productsRes = await fetch(
  `${SERVERURL}/api/app/medisoft/products/${targetShop.shopid}`,
  {
    method: "GET",
    signal: abortController.signal,
  }
        ...
        if (productsRes.ok) {
          ...
          if (Array.isArray(data)) {
            ...
            console.warn(
              "⚠️ No products found in target store, trying fallback..."
            );

            products =
              await fetchAllProductsFallback(
                shops,
                targetStoreName,
          ...
          console.warn(
            "⚠️ Products API failed, trying fallback..."
          );
        ...
        console.log(
          `✅ Found ${normalizedProducts.length} products for ${targetStoreName}`
        );
    ...
    async function fetchAllProductsFallback(
      ...
      console.log(
        "🔄 Fetching from all shops as fallback..."
      );
      ...
      for (const shop of shops) {
        try {
          const res = await fetch(
            `${SERVERURL}/api/app/medisoft/products`,
            {
              method: "POST",
              headers: {
          ...
          console.error(
            `Error fetching from shop ${shop.shopid}:`,
            err
          );
        }
      }
      ...
      console.log(
        `✅ Fallback found ${allProducts.length} products`
      );
  ...
  useEffect(() => {
    ...
    if (selectedStore) {
      storeFiltered = allProducts.filter(
        (p) => p.storeId === selectedStore.value
      );
    }

    if (!debouncedTerm.trim()) {
      setFilteredProducts(
        storeFiltered.slice(0, 6)
      );
  ...
  const addToCart = (product, e) => {
    ...
    const matchingStores = allProducts.filter(
      (p) =>
        (
          p.productName ||
          p.productname ||
    ...
    const filteredStores =
      matchingStores.filter(
        (storeProduct) =>
          storeProduct.storeName?.toUpperCase() ===
          targetStoreName.toUpperCase()
      );
    ...
    if (filteredStores.length === 0) {
      alert(
        `This product is not available in ${targetStoreName}`
      );
    ...
    const storeItems = filteredStores.map(
      (storeProduct) => ({
            ...
            >
              <Select
                ...
                styles={{
                  ...
                  // Forces the selected value text to dark
